...
Voronoi::Voronoi(float minDistanceBetweenSites)
{
	siteIdx = 0;
...
std::vector<GraphEdge*> Voronoi::generateVoronoi(std::vector<glm::vec2> points, float minX, float maxX, float minY, float maxY)
{
	sort(points);
	//original implementation includes check for mins bigger than maxes, don't bother with that for now
	borderMinX = minX;
	borderMaxX = maxX;
	borderMinY = minY;
	borderMaxY = maxY;
...
void Voronoi::pushGraphEdge(Site* leftSite, Site* rightSite, float x1, float y1, float x2, float y2)
{
	...
	//add corners
	newEdge->c1 = addCorner(leftSite, rightSite, x1, y1);
	newEdge->c2 = addCorner(leftSite, rightSite, x2, y2);
...
Corner* Voronoi::addCorner(Site* s1, Site* s2, float x, float y)
{
